...
def run_cmsDriver(configdata, label, release_path):
    ...
    command = ("echo $PWD; " + "cd " + release_path + "; shopt -s expand_aliases; set +u && source /cvmfs/cms.cern.ch/cmsset_default.sh; set -u; " +
        ...
        for key, value in conf.items():
            ...
            if key==label:
              ...
              if os.path.exists(f"hgcal_tpg_validation_{confName}_{label}_USER.py"):
                ...
                sourceProc = subprocess.run(sourceCmd, stdout=logfile, stderr=logfile, shell=True, check=True, text=True)
                #os.system(command)
            else:
