...
# Run cmsDriver
def run_cmsDriver(configdata, release, config):
    ...
    if procModifiers == 'empty':
        ...
        cmsDriver.py hgcal_tpg_validation_{config} -n {str(nbEvents)} \
...
def main(subsetconfig, release, config):
    ...
    # read th config file following the matrix elements
    print("Read config from the matrix")  
    config_data=read_config(config)
    
    # The configuration data will be used to generate a python script by cmsDriver 
    # and run the simulation+validation 
    command = run_cmsDriver(config_data, release, config)
...
if __name__ == "__main__":
    ...
    parser.add_option('--config', dest='config', help=' ', default='default_default')
    (opt, args) = parser.parse_args()
   
    main(opt.subsetconfig, opt.release, opt.config)
